...
specialForms.set = (args, scope) => {
    ...
    console.log(args);
    console.log(scope);
    console.log(Object.prototype.hasOwnProperty.call(scope, scope[args[0].name]));
    console.log(scope);
    ...
    // If the binding exists in the inner scope, throw error.
    if (Object.prototype.hasOwnProperty.call(scope, scope[args[0].name])) {
        throw new ReferenceError("The binding already exists in the inner scope");
    }

    // Get the outer scope.
    let outer_scope = Object.getPrototypeOf(scope);

    
    if (outer_scope) {
        // If the binding doesn't exist in the outer scope, throw error.
        // Note that 'the outer scope' means the nearest scope from the inner scope, not farthest scope.
        if (!Object.prototype.hasOwnProperty.call(outer_scope, args[0].name)) {
            throw new ReferenceError("The binding does not exist in the outer scope");
        }
    }

    let value = evaluate(args[1], scope);
    // Change the value of the binding.
    outer_scope[args[0].name] = value;
    return value;
}
...
run(`
    do(define(total, 0),
        define(count, 1),
        define(sum, fun(a, set(total, +(a, total)))),
        while(<(count, 11),
            do(sum(count),
                define(count, +(count, 1)))),
        print(total))
...
// Failed Pattern (No binding in outer scope)
// console.log("Failed Pattern (No binding in outer scope)");
// run(`
//     do(define(count, 1),
//         define(sum, fun(a, set(total, +(a, total)))),
//         while(<(count, 11),
