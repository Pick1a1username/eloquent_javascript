...
// Define the function for array type.
...
// Define the function to get an array's length.
topScope.length = array => {
    return array.length;
}

// Define the function to fetch the nth element from an array.
topScope.element = (array, idx) => {
    return array[idx];
}

run(`
    do(define(count, 0),
        define(array, array(1, 2, 3, 4 ,5)),
        print(length(array)),
        while(<(count, 5),
            do(print(element(array, count)),
            define(count, +(count, 1))
        )
        )
    )
